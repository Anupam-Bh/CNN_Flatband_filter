...
###################################
path = 'C:/Users/Anupam/Desktop/MP_flatband/flatness_code/5_create_fingerprint/'
list_files = glob.glob(path+'CrystalNN_fingerprint_from_DOS_*')
list_files = [a.replace('.','_') for a in list_files]
starts = [int(a.split('_')[-3]) for a in list_files]
ends = [int(a.split('_')[-2]) for a in list_files]
starts.sort()
ends.sort()
X=[]
indices=[]
...
for i in range(0,len(starts)):
    filename= path + 'CrystalNN_fingerprint_from_DOS_'+str(starts[i])+'_'+str(ends[i])+'.json'
    f = open (filename, "r")
    pred_data = json.load(f)
    f.close()
    for i in pred_data.keys():
        X.append(pred_data[i]['structure_fingerprint'])
        indices.append(i)
        sublattice.append(pred_data[i]['sublattice_element'])
        sublattice_score.append(pred_data[i]['primary_sublattice_score'])
X=np.array(X)
indices=np.array(indices)
#%%
# X= genfromtxt('../Reduced_lattice_fingerprint_from_DOS/input_flat_materials_96-96_DEEPNN_22feb2023.csv', delimiter=',')
# indices=genfromtxt('../Reduced_lattice_fingerprint_from_DOS/input_indices_96-96_DEEPNN_22feb2023.csv', delimiter=',')
# sublattice = np.transpose(pandas.read_csv('../Reduced_lattice_fingerprint_from_DOS/sublattice_element_96-96_DEEPNN_22feb2023.csv', header=None).values)[0]
# sublattice_score = np.transpose(pandas.read_csv('../Reduced_lattice_fingerprint_from_DOS/sublattice_element_96-96_DEEPNN_22feb2023.csv', header=None).values)[1]
# #print(np.where(np.isnan(sublattice_score)))
# print(type(sublattice_score))
# #colo=np.log(sublattice_score)
# with np.printoptions(threshold=np.inf):
#     print(sublattice_score)
database_index=[int(index.split('-')[1]) for index in indices]


################################################################
####  Read all materials data with flatness prediction
################################
#data = pandas.read_csv('../../write_and_plot_clusters_calculate_flatness_score/All_mat_new_test_DeepNN_96-96_10oct_2022.csv')
...
###################################
####  HDBSCAN
MS= 14     # minimum value of min_size
SS= 14     # minimum value of min_samp

f = open('DEEPNN_HDBSCAN_244_'+str(MS)+'_'+str(SS)+'_summary.txt', 'w')
f.write('Minkowski_metric_p=1\n')
...
for min_size in range(MS,MS+1,1):
    ...
    for min_samp in range(SS,SS+1,1):
        ...
        fname='DEEPNN_HDBSCAN_244_DOS_minsize_'+str(min_size)+'_minsamp_'+str(min_samp)
        ...
        #                gen_min_span_tree=False, leaf_size=40, metric='minkowski', cluster_selection_method='eom', min_cluster_size=min_size, min_samples=min_samp, p=1)
        hdb=HDBSCAN(algorithm='auto', alpha=1.0, leaf_size=40, metric='l2', cluster_selection_method='eom', min_cluster_size=min_size, min_samples=min_samp, n_jobs = -1, store_centers = 'both')
        db=hdb.fit(X)
        labels = db.labels_
        ...
        f.write(str(len(X))+'   '+str(min_size)+'   '+str(min_samp)+'   '+str(n_clusters_)+\
                '   '+str(n_noise_)+'   '+str(max(hist_labels_new))+'   '+str(metrics.silhouette_score(X, labels))+\
        ...
        # plt.yscale('log')
        # one.grid(True)
        # cond_plot_fname=fname+'_cond_tree_DEEPNN.png'
        ...
        #     name='mp-'+ str(database_index[i])
        #     ind=data.index[data['ID'] == name][0]
        #     data_all.append(data.loc[ind].values)
        # data_all=np.array(data_all)
        clubbed_data=np.transpose(np.vstack((indices,labels,cluster_size,sublattice,dbcv_score[1][labels],probabilities)))
        Out=pandas.DataFrame(clubbed_data,columns=['MP_index','Cluster_ID','cluster_size','sublattice_element','dbcv_cluster','probabilities'])
        Out.to_csv(fname+'_detailed_output_DeepNN.csv')
        ...
        #     if unique_label[j]==-1:
        #         nn=np.array([unique_label[j],counts[j],'none','none'])
        #     else: 
        #         nn=np.array([int(unique_label[j]),int(counts[j]),cluster_persistence[j-1],dbcv_score[1][j+1]])
        #     clusters.append(nn)
        # Out1=pandas.DataFrame(data=np.array(clusters),columns=['cluster_index','cluster_size','cluster_exemplar','cluster_dbcv'])
        # Out1.to_csv(fname+'_cluster_detail_DeepNN.csv')    
        ...
        tsne = manifold.TSNE(n_components=n_components, early_exaggeration=12.0,init="pca",learning_rate='auto', random_state=0,perplexity=30,n_iter=10000,verbose=2,n_jobs=-1)
        ...
        c=labels
        #c[labels==-1]=-5
        plt.scatter(objct[:,0], objct[:,1],s=2, c=labels, cmap="viridis")
        ...
        for rep_id in cluster_rep_index:
            ...
            plt.annotate(labels[rep_id],objct[rep_id,:]+[3.5,0],color=col,alpha=1, weight='normal', ha='center', va='center', size=12).draggable()
        ...
        #plt.show()
        #plt.savefig(fname+'_tsne.svg', format = 'svg', dpi=300)
        plt.savefig(fname+'_tsne_perplex30_auto.png', bbox_inches='tight')
        plt.show()
        plt.close(fig=fig)
f.close()
